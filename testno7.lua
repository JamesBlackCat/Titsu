...
local function setOrbVisibility(data, visible)
    ...
    -- Active and placed preview orbs stay queryable. The cursor placement
    -- ghost is visual only; respawning/dead orbs are removed from queries.
    data.part.CanQuery = not data.cursorPreview
        and (data.state == "Active" or data.state == "Preview")
...
local function refreshOrbAppearance(data)
    ...
    elseif data.state == "Preview" then
        data.part.Color = data.cursorPreview
            and Color3.fromRGB(255, 218, 92)
            or Color3.fromRGB(246, 188, 82)
        setOrbVisibility(data, true)
    elseif fraction < 0.35 then
        data.part.Color = Color3.fromRGB(255, 100, 80)
        -- Saved orb rigs remain visible so a spawned mark is easy to locate.
        setOrbVisibility(data, true)
    else
...
local destroyOrbData

local function destroyPlacementPreview()
    if OrbSystem.placementPreview then
        destroyOrbData(OrbSystem.placementPreview)
        OrbSystem.placementPreview = nil
    end
end

local function createPlacementPreview()
    if OrbSystem.placementPreview then return OrbSystem.placementPreview end
    local data = {
    ...
    local position = OrbSystem.floatMode
        and getFloatPlacementPosition()
        or getOrbPlacementPosition()
    createOrbPart(position, data)
    OrbSystem.placementPreview = data
    return data
end
...
local function createPreviewOrb()
    ...
    local placementPreview = OrbSystem.placementPreview
    local position = placementPreview and placementPreview.part
        and placementPreview.part.Position
        or (OrbSystem.floatMode and getFloatPlacementPosition()
            or getOrbPlacementPosition())
    createOrbPart(position, data)
...
local function updateOrbs(dt)
    ...
    if OrbSystem.setupActive and OrbSystem.placementPreview then
        local placement = OrbSystem.floatMode
            and getFloatPlacementPosition()
            or getOrbPlacementPosition()
        if OrbSystem.placementPreview.part then
            OrbSystem.placementPreview.part.Position = placement
        end
    end
...
local function getEntityAimPart(entity)
    ...
    -- Target Part Rotation override. Sitting targets use their own weighted
    -- profile, so a normal target can use (for example) Head at 40% while the
    -- same target uses Head at 100% as soon as its Humanoid.Sit is true.
    ...
    if isSitting and isPlayer and S.playerSitRotation then
        activePart, activeMiss = playerActiveSitTP, playerActiveSitMiss
    elseif isSitting and isNPC and S.npcSitRotation then
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if S.playerTPEnabled then
        ...
        if _playerTPAccum >= rate
        or (playerActiveTP == nil and not playerActiveMiss) then
            _playerTPAccum = 0
    ...
    -- Sitting Player Target Part Rotation timer
    if S.playerSitRotation then
        ...
        if _playerSitTPAccum >= rate
        or (playerActiveSitTP == nil and not playerActiveSitMiss) then
            _playerSitTPAccum = 0
    ...
    if S.npcTPEnabled then
        ...
        if _npcTPAccum >= rate
        or (npcActiveTP == nil and not npcActiveMiss) then
            _npcTPAccum = 0
    ...
    -- Sitting NPC Target Part Rotation timer
    if S.npcSitRotation then
        ...
        if _npcSitTPAccum >= rate
        or (npcActiveSitTP == nil and not npcActiveSitMiss) then
            _npcSitTPAccum = 0
...
local openRotationPopup
local function openRotationPartMenu(anchor, selectedValue, onSelect)
    if openRotationPopup then
        openRotationPopup:Destroy()
        openRotationPopup = nil
    end

    local popup = Instance.new("Frame")
    popup.Name = "RotationPartPopup"
    popup.Size = UDim2.fromOffset(190, 260)
    popup.BackgroundColor3 = COLORS.background
    popup.BorderSizePixel = 0
    popup.ZIndex = 100
    ...
    addCorner(popup, 9)
    addStroke(popup, COLORS.accentDark, 1)
    openRotationPopup = popup

    local viewport = Camera.ViewportSize
    local popupWidth, popupHeight = 190, 260
    local x = math.clamp(anchor.AbsolutePosition.X, 8, math.max(8, viewport.X - popupWidth - 8))
    local y = anchor.AbsolutePosition.Y + anchor.AbsoluteSize.Y + 4
    if y + popupHeight > viewport.Y - 8 then
        y = math.max(8, anchor.AbsolutePosition.Y - popupHeight - 4)
    end
    popup.Position = UDim2.fromOffset(x, y)

    local title = makeLabel(popup, "SELECT TARGET PART",
        UDim2.new(1, -44, 0, 28), COLORS.text, Enum.Font.GothamBold)
    ...
    close.Activated:Connect(function()
        popup:Destroy()
        if openRotationPopup == popup then openRotationPopup = nil end
    end)
    ...
    for _, option in ipairs(COMMON_PARTS) do
        local normalized = option == "(none)" and "" or option
        local row = Instance.new("TextButton")
        row.Size = UDim2.new(1, -4, 0, 28)
        row.BackgroundColor3 = normalized == selectedValue
            and COLORS.accentDark or COLORS.panelAlt
        row.BorderSizePixel = 0
        row.Text = option
        row.TextColor3 = COLORS.text
...
local function makeRotationEditor(page, titleText, prefix, enabledKey, rateKey, unitKey, missKey)
    ...
    for index = 1, 5 do
        ...
        partButton.Activated:Connect(function()
            openRotationPartMenu(partButton, S[prefix .. index .. "Part"], function(value)
                for optionIndex, option in ipairs(options) do
                    local normalized = option == "(none)" and "" or option
