...
local function setOrbVisibility(data, visible)
    ...
    data.part.Transparency = visible and 0.08 or 1
    -- Active orbs stay queryable even though their rig is invisible.  Preview
    -- orbs are queryable too so the same target code can inspect them while
    -- placing; respawning/dead orbs are removed from queries.
    data.part.CanQuery = data.state == "Active" or data.state == "Preview"
    data.part.CanTouch = false
    data.part:SetAttribute("OrbActive", data.state == "Active")
    data.part:SetAttribute("OrbVisual", visible)
    if data.light then data.light.Enabled = visible end
end
...
local function refreshOrbAppearance(data)
    ...
    if data.model then
        data.model:SetAttribute("OrbTarget", true)
        data.model:SetAttribute("OrbHP", hp)
        data.model:SetAttribute("OrbMaxHP", maxHp)
        data.model:SetAttribute("OrbState", data.state or "Active")
        data.model:SetAttribute("OrbActive", data.state == "Active")
    end
    ...
    elseif fraction <= 0 then
        ...
        setOrbVisibility(data, false)
    elseif data.state == "Preview" then
        data.part.Color = Color3.fromRGB(246, 188, 82)
        setOrbVisibility(data, true)
    elseif fraction < 0.35 then
        data.part.Color = Color3.fromRGB(255, 100, 80)
        setOrbVisibility(data, S.orbVisible == true)
    else
...
local function markOrbDead(data)
    if not data or data.state ~= "Active" then return end
    data.hp = 0
    if S.orbRespawnEnabled == true then
        data.state = "Respawning"
        data.respawnAt = tick() + math.max(0, tonumber(S.orbRespawnTime) or 0)
    else
        data.state = "Dead"
    end
...
local function createOrbPart(position, data)
    ...
    rig.Name = "OrbRig"
    rig:SetAttribute("OrbRig", true)
    rig:SetAttribute("OrbTarget", true)
    ...
    humanoid.Name = "OrbHealth"
    humanoid.MaxHealth = math.max(1, tonumber(data.maxHp) or 1)
    humanoid.Health = math.clamp(
        tonumber(data.hp) or humanoid.MaxHealth, 0, humanoid.MaxHealth)
    ...
    data.healthConnection = humanoid.HealthChanged:Connect(function(health)
        if data._syncingHealth or data.state ~= "Active" then return end
        data.hp = math.clamp(tonumber(health) or 0, 0, data.maxHp)
        if data.hp <= 0 then
            markOrbDead(data)
            return
        end
    ...
    data.deathConnection = humanoid.Died:Connect(function()
        markOrbDead(data)
    end)
    refreshOrbAppearance(data)
    return part
...
local function createPreviewOrb()
    if not S.orbEnabled or S.orbDeleteMode then return nil end
    if OrbSystem.cursorPreview then
        OrbSystem.cursorPreview.cursorPreview = false
        OrbSystem.cursorPreview = nil
    end
    local data = {
    ...
    OrbSystem.cursorPreview = data
    if OrbSystem.syncSetupControls then OrbSystem.syncSetupControls() end
    return data
end
...
local function destroyOrbData(data)
    ...
    if data and data.healthConnection then
        pcall(function() data.healthConnection:Disconnect() end)
        data.healthConnection = nil
    end
    if data and data.deathConnection then
        pcall(function() data.deathConnection:Disconnect() end)
        data.deathConnection = nil
    end
...
local function undoPreviewOrb()
    for index = #OrbSystem.preview, 1, -1 do
        local data = OrbSystem.preview[index]
        if data ~= OrbSystem.cursorPreview then
            table.remove(OrbSystem.preview, index)
            destroyOrbData(data)
            break
        end
    end
    if OrbSystem.setupActive and not OrbSystem.cursorPreview then
        createPreviewOrb()
    end
    if OrbSystem.syncSetupControls then OrbSystem.syncSetupControls() end
end
...
local function setPreviewOrbs()
    ...
    for _, data in ipairs(OrbSystem.preview) do
        ...
        if data.humanoid then
            ...
            data.humanoid.MaxHealth = data.maxHp
            data.humanoid.Health = data.hp
...
local function beginOrbSetup()
    if not S.orbEnabled or S.orbDeleteMode then return end
...
local function deleteOrb(data)
    ...
    if OrbSystem.cursorPreview == data then OrbSystem.cursorPreview = nil end
    destroyOrbData(data)
    if OrbSystem.syncSetupControls then OrbSystem.syncSetupControls() end
end
...
local function updateOrbs(dt)
    ...
    if OrbSystem.setupActive and OrbSystem.cursorPreview then
        local preview = OrbSystem.cursorPreview
        if preview.part then
            preview.part.Position = OrbSystem.floatMode
                and getFloatPlacementPosition()
                or getOrbPlacementPosition()
        end
    end

    local now = tick()
    for index = #OrbSystem.saved, 1, -1 do
        local data = OrbSystem.saved[index]
        if data.state == "Dead"
        or (data.state == "Respawning" and S.orbRespawnEnabled ~= true) then
            table.remove(OrbSystem.saved, index)
            destroyOrbData(data)
        elseif data.state == "Respawning" and now >= (data.respawnAt or now) then
            data.state = "Active"
            data.hp = data.maxHp
            if data.humanoid then
                data._syncingHealth = true
...
_G.__Aimlock_DamageOrb = function(target, amount)
    local data = getOrbData(target)
    if not data and typeof(target) == "Instance" then
        if target:IsA("Model") then
            data = getOrbData(target.PrimaryPart
                or target:FindFirstChild("HumanoidRootPart"))
        elseif target:IsA("Humanoid") and target.Parent then
            local model = target.Parent
            data = getOrbData(model.PrimaryPart
                or model:FindFirstChild("HumanoidRootPart"))
        end
    end
    return damageOrb(data, amount)
end
...
local function tryAddNPC(model)
    ...
    if OrbSystem.folder and model:IsDescendantOf(OrbSystem.folder) then return end
...
local playerActiveSitTP   = nil
local npcActiveSitTP      = nil
local playerActiveSitMiss = false
local npcActiveSitMiss    = false
...
playerActiveSitTP, playerActiveSitMiss =
    pickWeightedPart("playerSitTP", S.playerSitTPMiss)
npcActiveSitTP, npcActiveSitMiss =
...
local function getEntityAimPart(entity)
    ...
    -- A seated target uses its own weighted rotation table.  The table has
    -- the same five part/chance rows as standing rotation, so sitting can
    -- change a target from (for example) Head 40% to Head 100% without
    -- changing the standing configuration.
    local hum = model:FindFirstChildWhichIsA("Humanoid", true)
    local sitting = hum and hum.Sit == true
    local partName
    local activePart
    local activeMiss
    ...
    if entity:IsA("Player") then
        if sitting and S.playerSitRotation then
            activePart, activeMiss = playerActiveSitTP, playerActiveSitMiss
            rotationEnabled = true
        else
            activePart, activeMiss = playerActiveTP, playerActiveMiss
            rotationEnabled = S.playerTPEnabled
        end
    elseif not entity:IsA("BasePart") then
        if sitting and S.npcSitRotation then
            activePart, activeMiss = npcActiveSitTP, npcActiveSitMiss
            rotationEnabled = true
        else
            activePart, activeMiss = npcActiveTP, npcActiveMiss
            rotationEnabled = S.npcTPEnabled
        end
    end
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if S.playerSitRotation then
        ...
        _playerSitTPAccum = 0
        playerActiveSitTP = nil
        playerActiveSitMiss = false
    end

    -- NPC sitting rotation timer
...
-- ── Quick orb setup controls ──────────────────────────────────
-- This stays outside the control center so closing the settings window never
-- interrupts placement.  It only appears while setup is active.
local orbQuickBar = Instance.new("Frame")
orbQuickBar.Name = "OrbSetupQuickBar"
orbQuickBar.AnchorPoint = Vector2.new(0.5, 1)
orbQuickBar.Position = UDim2.new(0.5, 0, 1, -20)
orbQuickBar.Size = UDim2.new(0.92, 0, 0, 116)
orbQuickBar.BackgroundColor3 = COLORS.background
orbQuickBar.BackgroundTransparency = 0.06
orbQuickBar.BorderSizePixel = 0
orbQuickBar.Visible = false
orbQuickBar.ZIndex = 70
orbQuickBar.Parent = ControlGui
addCorner(orbQuickBar, 12)
addStroke(orbQuickBar, COLORS.accentDark, 1)

local quickBarConstraint = Instance.new("UISizeConstraint")
quickBarConstraint.MinSize = Vector2.new(280, 116)
quickBarConstraint.MaxSize = Vector2.new(520, 116)
quickBarConstraint.Parent = orbQuickBar

local orbQuickTitle = makeLabel(orbQuickBar, "ORB SETUP", UDim2.new(0.5, 0, 0, 22),
    COLORS.accent, Enum.Font.GothamBold)
orbQuickTitle.Position = UDim2.new(0, 12, 0, 7)
orbQuickTitle.TextSize = 10
orbQuickTitle.ZIndex = 71

local orbQuickStatus = makeLabel(orbQuickBar, "0 preview  •  0 saved",
    UDim2.new(0.5, -12, 0, 22), COLORS.muted, Enum.Font.GothamSemibold)
orbQuickStatus.Position = UDim2.new(0.5, 0, 0, 7)
orbQuickStatus.TextSize = 10
orbQuickStatus.TextXAlignment = Enum.TextXAlignment.Right
orbQuickStatus.ZIndex = 71

local function makeQuickRow(y, height)
    local row = Instance.new("Frame")
    row.Size = UDim2.new(1, -20, 0, height)
    row.Position = UDim2.new(0, 10, 0, y)
    row.BackgroundTransparency = 1
    row.ZIndex = 71
    row.Parent = orbQuickBar
...
local function makeQuickButton(row, labelText, color, callback, count)
    local button = Instance.new("TextButton")
    button.Size = UDim2.new(1 / (count or 5), -5, 1, 0)
    button.BackgroundColor3 = color
    ...
    button.MouseEnter:Connect(function()
        button.BackgroundColor3 = color:Lerp(Color3.new(1, 1, 1), 0.08)
    end)
...
local quickActions = makeQuickRow(32, 34)
makeQuickButton(quickActions, "START", COLORS.accentDark, beginOrbSetup)
makeQuickButton(quickActions, "SPAWN", COLORS.good, function()
    if not OrbSystem.setupActive then beginOrbSetup() end
    createPreviewOrb()
    if OrbSystem.syncSetupControls then OrbSystem.syncSetupControls() end
end)
makeQuickButton(quickActions, "UNDO", COLORS.warning, function()
    undoPreviewOrb()
    if OrbSystem.syncSetupControls then OrbSystem.syncSetupControls() end
end)
makeQuickButton(quickActions, "SET", COLORS.good, setPreviewOrbs)
makeQuickButton(quickActions, "CANCEL", COLORS.danger, cancelOrbSetup)

local quickMotion = makeQuickRow(76, 30)
local quickFloat = makeQuickButton(quickMotion, "FLOAT", COLORS.accentDark, function()
    OrbSystem.floatMode = not OrbSystem.floatMode
    if OrbSystem.floatMode and OrbSystem.setupActive and #OrbSystem.preview == 0 then
        createPreviewOrb()
    end
    if OrbSystem.syncSetupControls then OrbSystem.syncSetupControls() end
end, 3)
local quickForward = makeQuickButton(quickMotion, "FORWARD (HOLD)", COLORS.panelAlt,
    function() end, 3)
local quickBackward = makeQuickButton(quickMotion, "BACKWARD (HOLD)", COLORS.panelAlt,
    function() end, 3)

local function isQuickHoldInput(input)
    return input.UserInputType == Enum.UserInputType.MouseButton1
        or input.UserInputType == Enum.UserInputType.Touch
end
quickForward.InputBegan:Connect(function(input)
    if isQuickHoldInput(input) then OrbSystem.forwardHeld = true end
end)
quickForward.InputEnded:Connect(function(input)
    if isQuickHoldInput(input) then OrbSystem.forwardHeld = false end
end)
quickBackward.InputBegan:Connect(function(input)
    if isQuickHoldInput(input) then OrbSystem.backwardHeld = true end
end)
quickBackward.InputEnded:Connect(function(input)
    if isQuickHoldInput(input) then OrbSystem.backwardHeld = false end
end)

OrbSystem.syncSetupControls = function()
    local active = OrbSystem.setupActive and S.orbEnabled and not S.orbDeleteMode
    orbQuickBar.Visible = active
    orbQuickStatus.Text = string.format("%d preview  •  %d saved",
        #OrbSystem.preview, #OrbSystem.saved)
    quickFloat.Text = OrbSystem.floatMode and "FLOAT: ON" or "FLOAT"
    quickFloat.BackgroundColor3 = OrbSystem.floatMode and COLORS.good or COLORS.accentDark
    if not active then
        OrbSystem.forwardHeld = false
        OrbSystem.backwardHeld = false
    end
end
OrbSystem.syncSetupControls()
...
local function openSingleChoicePopup(titleText, options, currentValue, onSelected)
    if openListPopup then
        openListPopup:Destroy()
        openListPopup = nil
    end

    local popup = Instance.new("Frame")
    popup.Name = "SingleChoicePopup"
    popup.Size = UDim2.new(0, 230, 0, 300)
    popup.Position = UDim2.new(0, 122, 0, 45)
    popup.BackgroundColor3 = COLORS.background
    popup.BorderSizePixel = 0
    popup.ZIndex = 60
    ...
    local titleLabel = makeLabel(popup, titleText, UDim2.new(1, -54, 0, 28),
        COLORS.text, Enum.Font.GothamBold)
    titleLabel.Position = UDim2.new(0, 12, 0, 5)
    titleLabel.TextSize = 11
    titleLabel.ZIndex = 61
    ...
    for _, option in ipairs(options) do
        local row = Instance.new("TextButton")
        row.Size = UDim2.new(1, -4, 0, 30)
        row.BackgroundColor3 = option == currentValue
            and COLORS.accentDark or COLORS.panelAlt
        row.BorderSizePixel = 0
        row.Text = (option == currentValue and "✓  " or "    ") .. option
        row.TextColor3 = COLORS.text
        row.TextSize = 10
        row.Font = Enum.Font.GothamSemibold
        row.TextXAlignment = Enum.TextXAlignment.Left
...
local function makeRotationEditor(page, titleText, prefix, enabledKey, rateKey, unitKey, missKey)
    ...
    for index = 1, 5 do
        ...
        partButton.Activated:Connect(function()
            openSingleChoicePopup("Select target part", options, options[selectedIndex],
                function(selected)
                    for optionIndex, option in ipairs(options) do
...
addToggle(orbPage, "Show active orb visuals", S.orbVisible, function(v)
    S.orbVisible = v
    for _, data in ipairs(OrbSystem.saved) do refreshOrbAppearance(data) end
end, "Visible glowing orb markers; disabling keeps them targetable but hidden")
...
addToggle(orbPage, "Respawn after death", S.orbRespawnEnabled, function(v)
    S.orbRespawnEnabled = v
    if not v then
        for index = #OrbSystem.saved, 1, -1 do
            local data = OrbSystem.saved[index]
            if data.state == "Dead" or data.state == "Respawning" then
                deleteOrb(data)
            end
        end
    end
end, "OFF deletes an orb permanently when its HP reaches zero")
...
addAction(orbPage, "DELETE ALL ORBS", function()
    for index = #OrbSystem.saved, 1, -1 do
        deleteOrb(OrbSystem.saved[index])
    end
    clearPreviewOrbs()
    OrbSystem.selected = {}
    if OrbSystem.syncSetupControls then OrbSystem.syncSetupControls() end
...
makeRotationEditor(rotationPage, "Player Sitting Target Part Rotation", "playerSitTP",
    "playerSitRotation", "playerSitTPRate", "playerSitTPUnit", "playerSitTPMiss")
makeRotationEditor(rotationPage, "NPC Sitting Target Part Rotation", "npcSitTP",
