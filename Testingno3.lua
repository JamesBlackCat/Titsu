...
local D = {
    ...
    playerTP5Chance    = 0,
    playerTPMiss       = false,
    -- ── Target Part Rotation — sitting players ────────────────
    playerSitTPRate    = 500,
    playerSitTPUnit    = "ms",
    ...
    npcTP5Chance       = 0,
    npcTPMiss          = false,
    -- ── Target Part Rotation — sitting NPCs ───────────────────
    npcSitTPRate       = 500,
    npcSitTPUnit       = "ms",
    npcSitTP1Part      = "Head",
    npcSitTP1Chance    = 100,
...
local showOrbSetupToolbar
local hideOrbSetupToolbar
...
local function setOrbVisibility(data, visible)
    ...
    local preview = data.state == "Preview"
    data.part.Transparency = 1
    data.part.CanQuery = visible and not preview
    data.part.CanTouch = false
    data.part:SetAttribute("OrbActive", visible)
    if data.ghost then
        data.ghost.Transparency = visible and preview and 0.6 or 1
        data.ghost.CanQuery = false
    end
end
...
local function refreshOrbAppearance(data)
    ...
    if data.state == "Respawning" then
        ...
        if data.ghost then data.ghost.Color = data.part.Color end
...
local function createOrbPart(position, data)
    ...
    ghost.Size = Vector3.new(S.orbSize, S.orbSize, S.orbSize)
...
local function updateOrbGhost(data)
    if data and data.ghost and data.part then
        data.ghost.Position = data.part.Position
    end
end
...
local function destroyOrbData(data)
    ...
    elseif data and data.part then
...
local function setPreviewOrbs()
    ...
    if hideOrbSetupToolbar then hideOrbSetupToolbar() end
end

local function beginOrbSetup()
    OrbSystem.setupActive = true
    clearPreviewOrbs()
    if showOrbSetupToolbar then showOrbSetupToolbar() end
end
...
local function updateOrbs(dt)
    ...
    if OrbSystem.setupActive and OrbSystem.floatMode then
        for _, data in ipairs(OrbSystem.preview) do
            if data.part then
                data.part.Position = getFloatPlacementPosition()
                updateOrbGhost(data)
            end
        end
    end
...
local function isEntitySitting(entity)
    local model = getEntityModel(entity)
    if not model or model:IsA("BasePart") then return false end
    local hum = model:FindFirstChildWhichIsA("Humanoid", true)
...
local function getEntityAimPart(entity)
    ...
    local sitting = isEntitySitting(entity)
    local rotationPrefix
    local activePart
    local activeMiss
    local partName
    if entity:IsA("Player") then
        if sitting and S.playerSitRotation then
            rotationPrefix, activePart, activeMiss =
                "playerSitTP", playerSitActiveTP, playerSitActiveMiss
        elseif S.playerTPEnabled then
            rotationPrefix, activePart, activeMiss =
                "playerTP", playerActiveTP, playerActiveMiss
        end
    elseif not entity:IsA("BasePart") then
        if sitting and S.npcSitRotation then
            rotationPrefix, activePart, activeMiss =
                "npcSitTP", npcSitActiveTP, npcSitActiveMiss
        elseif S.npcTPEnabled then
            rotationPrefix, activePart, activeMiss =
                "npcTP", npcActiveTP, npcActiveMiss
        end
    end
    if rotationPrefix then
        if activeMiss then return nil end
        partName = (activePart ~= nil and activePart ~= "") and activePart or nil
    end
...
local activePartMenu
local function openPartSelector(anchor, options, selectedIndex, callback)
    if activePartMenu then activePartMenu:Destroy() activePartMenu = nil end

    local menu = Instance.new("Frame")
    menu.Name = "PartSelectorMenu"
    ...
    for index, option in ipairs(options) do
        ...
        row.BackgroundColor3 = index == selectedIndex and COLORS.accentDark or COLORS.panelAlt
        row.BorderSizePixel = 0
        row.Text = option
        row.TextColor3 = COLORS.text
        row.TextSize = 10
        ...
        row.Activated:Connect(function()
            ...
            menu:Destroy()
            if activePartMenu == menu then activePartMenu = nil end
        end)
    end
end
...
local function makeRotationEditor(page, titleText, prefix, enabledKey, rateKey, unitKey, missKey)
    ...
    for index = 1, 5 do
        ...
        partButton.Activated:Connect(function()
            openPartSelector(partButton, options, selectedIndex, function(nextIndex)
...
local toolbarScale
local orbToolbar
local orbToolbarFloatRow
local orbToolbarFloatButton

do
    orbToolbar = Instance.new("Frame")
    orbToolbar.Name = "OrbSetupToolbar"
    orbToolbar.AnchorPoint = Vector2.new(0.5, 1)
    orbToolbar.Position = UDim2.new(0.5, 0, 1, -24)
    orbToolbar.Size = UDim2.new(0, 430, 0, 62)
    orbToolbar.BackgroundColor3 = COLORS.background
    orbToolbar.BorderSizePixel = 0
    orbToolbar.Visible = false
    orbToolbar.ZIndex = 45
    orbToolbar.Parent = ControlGui
    addCorner(orbToolbar, 12)
    addStroke(orbToolbar, COLORS.accentDark, 1)

    toolbarScale = Instance.new("UIScale")
    toolbarScale.Name = "ResponsiveScale"
    toolbarScale.Scale = windowScale.Scale
    toolbarScale.Parent = orbToolbar

    local toolbarTitle = makeLabel(orbToolbar, "ORB SETUP", UDim2.new(0, 90, 0, 18),
        COLORS.accent, Enum.Font.GothamBold)
    toolbarTitle.Position = UDim2.new(0, 12, 0, 5)
    toolbarTitle.TextSize = 9
    toolbarTitle.ZIndex = 46

    local toolbarRow = Instance.new("Frame")
    toolbarRow.Size = UDim2.new(1, -20, 0, 34)
    toolbarRow.Position = UDim2.new(0, 10, 0, 23)
    toolbarRow.BackgroundTransparency = 1
    toolbarRow.ZIndex = 46
    toolbarRow.Parent = orbToolbar
    ...
    toolbarLayout.Parent = toolbarRow

    local function toolbarButton(text, color, callback)
        local button = Instance.new("TextButton")
        button.Size = UDim2.new(0, 74, 1, 0)
        button.BackgroundColor3 = color
        button.BorderSizePixel = 0
        button.Text = text
        button.TextColor3 = COLORS.text
        ...
        button.AutoButtonColor = false
        button.ZIndex = 47
        button.Parent = toolbarRow
        addCorner(button, 7)
        button.Activated:Connect(callback)
        return button
    end

    toolbarButton("SPAWN", COLORS.good, function()
        if not OrbSystem.setupActive then beginOrbSetup() end
        createPreviewOrb()
    end)
    toolbarButton("UNDO", COLORS.warning, undoPreviewOrb)
    toolbarButton("SET", COLORS.accentDark, setPreviewOrbs)
    toolbarButton("CANCEL", COLORS.danger, cancelOrbSetup)
    orbToolbarFloatButton = toolbarButton("FLOAT", COLORS.accentDark, function()
        OrbSystem.floatMode = not OrbSystem.floatMode
        if OrbSystem.floatMode and #OrbSystem.preview == 0 then createPreviewOrb() end
        if orbToolbarFloatRow then
            orbToolbarFloatRow.Visible = OrbSystem.floatMode
            orbToolbar.Size = UDim2.new(0, 430, 0, OrbSystem.floatMode and 104 or 62)
        end
    end)

    orbToolbarFloatRow = Instance.new("Frame")
    orbToolbarFloatRow.Name = "FloatControls"
    orbToolbarFloatRow.Size = UDim2.new(1, -20, 0, 34)
    orbToolbarFloatRow.Position = UDim2.new(0, 10, 0, 66)
    orbToolbarFloatRow.BackgroundTransparency = 1
    orbToolbarFloatRow.Visible = false
    orbToolbarFloatRow.ZIndex = 46
    orbToolbarFloatRow.Parent = orbToolbar
    ...
    floatLayout.Parent = orbToolbarFloatRow

    local function holdButton(text, direction)
        local button = Instance.new("TextButton")
        ...
        button.AutoButtonColor = false
        button.ZIndex = 47
        button.Parent = orbToolbarFloatRow
        addCorner(button, 7)
    ...
    holdButton("◀  BACKWARD (HOLD)", -1)
    holdButton("FORWARD (HOLD)  ▶", 1)
end

local function updateOrbToolbarState()
    if not orbToolbar then return end
    orbToolbar.Visible = OrbSystem.setupActive == true
    local isFloat = OrbSystem.floatMode == true
    if orbToolbarFloatRow then orbToolbarFloatRow.Visible = isFloat end
    if orbToolbarFloatButton then
        orbToolbarFloatButton.Text = isFloat and "FLOAT ON" or "FLOAT"
        orbToolbarFloatButton.BackgroundColor3 = isFloat and COLORS.good or COLORS.accentDark
    end
    orbToolbar.Size = UDim2.new(0, 430, 0, isFloat and 104 or 62)
end

showOrbSetupToolbar = updateOrbToolbarState
hideOrbSetupToolbar = updateOrbToolbarState
Camera:GetPropertyChangedSignal("ViewportSize"):Connect(function()
    if toolbarScale then toolbarScale.Scale = windowScale.Scale end
end)
...
local savePage = createPage("SaveLoad", "SAVE / LOAD")
...
floatActionRow.Visible = OrbSystem.floatMode == true
...
makeRotationEditor(rotationPage, "Player Sitting Rotation Override", "playerSitTP",
    "playerSitRotation", "playerSitTPRate", "playerSitTPUnit", "playerSitTPMiss")
makeRotationEditor(rotationPage, "NPC Sitting Rotation Override", "npcSitTP",
    "npcSitRotation", "npcSitTPRate", "npcSitTPUnit", "npcSitTPMiss")
...
-- ── Save / Load ──────────────────────────────────────────────
-- Saves are portable settings snapshots.  Orb instances and temporary
-- runtime state are deliberately not part of the payload.
local SAVE_FILE = "AimlockControlCenter_Saves.json"
local saveRecords = {}
local saveStorageMode = "Session only"

local function hasLocalSaveStorage()
    return type(readfile) == "function" and type(writefile) == "function"
end

local function encodeSaveValue(value)
    ...
    elseif type(value) == "table" then
        ...
        for key, item in pairs(value) do
            output[key] = encodeSaveValue(item)
        end
        return output
    end
    return value
...
local function decodeSaveValue(value)
    ...
    local output = {}
    for key, item in pairs(value) do output[key] = decodeSaveValue(item) end
    return output
end
...
local function buildSavePayload()
    ...
    for _, entry in ipairs(buttonRegistry) do
        floatingButtons[entry.sKey] = encodeSaveValue(entry.obj.frame.Position)
    end
    return {
        settings = encodeSaveValue(S),
        crosshair = encodeSaveValue(S.aimlockCrosshairPos),
        windowPosition = encodeSaveValue(window.Position),
        dButtonPosition = encodeSaveValue(dButton.Position),
        floatingButtons = floatingButtons,
    }
end

local function applySavePayload(payload)
    if not payload then return false end
    local decodedSettings = decodeSaveValue(payload.settings or {})
    for key, value in pairs(decodedSettings) do S[key] = value end

    local decodedCrosshair = decodeSaveValue(payload.crosshair)
    if decodedCrosshair and typeof(decodedCrosshair) == "UDim2" then
        S.aimlockCrosshairPos = decodedCrosshair
        applyCrosshairPosition(decodedCrosshair)
    end
    local savedWindowPosition = decodeSaveValue(payload.windowPosition)
    if savedWindowPosition and typeof(savedWindowPosition) == "UDim2" then
        window.Position = savedWindowPosition
    end
    local savedDPosition = decodeSaveValue(payload.dButtonPosition)
    if savedDPosition and typeof(savedDPosition) == "UDim2" then
        dButton.Position = savedDPosition
    end
    for _, entry in ipairs(buttonRegistry) do
        local encoded = payload.floatingButtons
            and payload.floatingButtons[entry.sKey]
        local position = decodeSaveValue(encoded)
        if position and typeof(position) == "UDim2" then
            entry.obj.frame.Position = position
            S[entry.sKey] = position
        end
    end
    ...
    if _G.__FlyScript_UpdateAimlockBtn then
        pcall(_G.__FlyScript_UpdateAimlockBtn)
    end
    return true
end

local function loadSaveRecords()
    if not hasLocalSaveStorage() then
        saveStorageMode = "Session only"
        return
    end
    local ok, content = pcall(readfile, SAVE_FILE)
    if not ok or type(content) ~= "string" or content == "" then
        saveStorageMode = "Local storage ready"
        return
    end
    ...
    if decodedOk and type(decoded) == "table" then
        saveRecords = decoded
        saveStorageMode = "Loaded from local storage"
    else
        saveStorageMode = "Local storage reset"
    end
end

local function writeSaveRecords()
    if not hasLocalSaveStorage() then
        saveStorageMode = "Session only"
        return false
    end
    local ok = pcall(function()
        writefile(SAVE_FILE, HttpService:JSONEncode(saveRecords))
    end)
    saveStorageMode = ok and "Saved locally" or "Session only"
    return ok
end
...
local function closeSaveOverlays()
    if saveDialog then saveDialog:Destroy() saveDialog = nil end
    if saveMenu then saveMenu:Destroy() saveMenu = nil end
end

local function openSaveDialog(recordIndex)
    closeSaveOverlays()
    local existing = recordIndex and saveRecords[recordIndex]
    local overlay = Instance.new("Frame")
    overlay.Name = "SaveDialog"
    overlay.Size = UDim2.new(1, 0, 1, 0)
    overlay.BackgroundColor3 = Color3.new(0, 0, 0)
    overlay.BackgroundTransparency = 0.35
    overlay.ZIndex = 80
    overlay.Parent = ControlGui
    saveDialog = overlay
    ...
    card.Parent = overlay
    addCorner(card, 10)
    addStroke(card, COLORS.accentDark, 1)

    local heading = makeLabel(card, existing and "RENAME SAVE" or "NEW SAVE",
        UDim2.new(1, -28, 0, 24), COLORS.text, Enum.Font.GothamBold)
    heading.Position = UDim2.new(0, 14, 0, 12)
    heading.TextSize = 12
    heading.ZIndex = 82
    local input = Instance.new("TextBox")
    input.Size = UDim2.new(1, -28, 0, 32)
    input.Position = UDim2.new(0, 14, 0, 45)
    input.BackgroundColor3 = COLORS.panelAlt
    input.BorderSizePixel = 0
    input.Text = existing and existing.name or "My Setup"
    input.PlaceholderText = "Save name"
    ...
    saveButton.Size = UDim2.new(0, 110, 0, 32)
    saveButton.Position = UDim2.new(0, 14, 1, -46)
    saveButton.BackgroundColor3 = COLORS.good
    saveButton.BorderSizePixel = 0
    saveButton.Text = existing and "RENAME" or "SAVE"
    saveButton.TextColor3 = COLORS.background
    saveButton.TextSize = 10
    saveButton.Font = Enum.Font.GothamBold
    saveButton.ZIndex = 82
    saveButton.Parent = card
    addCorner(saveButton, 6)

    local cancelButton = Instance.new("TextButton")
    cancelButton.Size = UDim2.new(0, 110, 0, 32)
    cancelButton.Position = UDim2.new(1, -124, 1, -46)
    cancelButton.BackgroundColor3 = COLORS.panelAlt
    ...
    cancelButton.Activated:Connect(closeSaveOverlays)
    saveButton.Activated:Connect(function()
        ...
        if existing then
            ...
            table.insert(saveRecords, { name = name, payload = buildSavePayload() })
        end
        writeSaveRecords()
        closeSaveOverlays()
        renderSaveCards()
...
local function openSaveMenu(recordIndex, anchor)
    if saveMenu then saveMenu:Destroy() saveMenu = nil end
    local menu = Instance.new("Frame")
    menu.Size = UDim2.new(0, 120, 0, 76)
    menu.BackgroundColor3 = COLORS.background
    menu.BorderSizePixel = 0
    menu.ZIndex = 90
    menu.Parent = ControlGui
    addCorner(menu, 7)
    addStroke(menu, COLORS.accentDark, 1)
    menu.Position = UDim2.fromOffset(anchor.AbsolutePosition.X,
        anchor.AbsolutePosition.Y + anchor.AbsoluteSize.Y + 4)
    saveMenu = menu
    ...
    local function menuButton(text, y, callback)
        ...
        button.Parent = menu
        addCorner(button, 5)
        button.Activated:Connect(function() callback(); if menu then menu:Destroy() end saveMenu = nil end)
    end
    menuButton("RENAME", 5, function() openSaveDialog(recordIndex) end)
    menuButton("DELETE", 39, function()
        table.remove(saveRecords, recordIndex)
        writeSaveRecords()
        renderSaveCards()
    end)
end
...
function renderSaveCards()
    for _, card in ipairs(saveCards) do pcall(function() card:Destroy() end) end
    saveCards = {}
    for index, record in ipairs(saveRecords) do
        local card = addCard(savePage, 66)
        card:SetAttribute("SaveCard", true)
        table.insert(saveCards, card)
        local nameLabel = makeLabel(card, tostring(record.name or "Unnamed setup"),
            UDim2.new(1, -170, 0, 20), COLORS.text, Enum.Font.GothamBold)
        nameLabel.Position = UDim2.new(0, 14, 0, 10)
        nameLabel.TextSize = 11
        local subLabel = makeLabel(card, "Settings snapshot — orbs excluded",
            UDim2.new(1, -170, 0, 16), COLORS.muted, Enum.Font.Gotham)
        subLabel.Position = UDim2.new(0, 14, 0, 32)
        subLabel.TextSize = 9
        local loadButton = Instance.new("TextButton")
        loadButton.Size = UDim2.new(0, 64, 0, 28)
        loadButton.Position = UDim2.new(1, -102, 0.5, -14)
        loadButton.BackgroundColor3 = COLORS.good
        loadButton.BorderSizePixel = 0
        loadButton.Text = "LOAD"
        loadButton.TextColor3 = COLORS.background
        loadButton.TextSize = 9
        loadButton.Font = Enum.Font.GothamBold
        loadButton.Parent = card
        addCorner(loadButton, 6)
        loadButton.Activated:Connect(function() applySavePayload(record.payload) end)
        local menuButton = Instance.new("TextButton")
        menuButton.Size = UDim2.new(0, 28, 0, 28)
        menuButton.Position = UDim2.new(1, -34, 0.5, -14)
        ...
        menuButton.Activated:Connect(function() openSaveMenu(index, menuButton) end)
    end
end

loadSaveRecords()
addSection(savePage, "Saved setups")
addAction(savePage, "+ NEW SAVE", function() openSaveDialog(nil) end, COLORS.accentDark)
local saveStatusCard = addCard(savePage, 42)
local saveStatus = makeLabel(saveStatusCard, saveStorageMode,
    UDim2.new(1, -28, 1, 0), COLORS.muted, Enum.Font.Gotham)
saveStatus.Position = UDim2.new(0, 14, 0, 0)
...
-- ── Startup credit animation ────────────────────────────────
-- The credit resolves into the same D launcher used to open the panel.
local function runStartupAnimation()
    window.Visible = false
    dButton.Visible = false

    local overlay = Instance.new("Frame")
    overlay.Name = "StartupCredits"
    overlay.Size = UDim2.new(1, 0, 1, 0)
    ...
    credit.Position = UDim2.new(0.5, 0, 0.5, 0)
    credit.Size = UDim2.new(0, 360, 0, 48)
    credit.BackgroundTransparency = 1
    credit.Text = "Made by DanielCheeseSauce"
    credit.TextColor3 = COLORS.text
    credit.TextTransparency = 1
    credit.TextSize = 24
    credit.Font = Enum.Font.GothamBold
    credit.ZIndex = 101
    credit.Parent = overlay

    local introCircle = Instance.new("TextButton")
    introCircle.AnchorPoint = Vector2.new(0.5, 0.5)
    introCircle.Position = UDim2.new(0.5, 0, 0.5, 0)
    introCircle.Size = UDim2.new(0, 112, 0, 112)
    introCircle.BackgroundColor3 = COLORS.accentDark
    introCircle.BorderSizePixel = 0
    introCircle.Text = "D"
    introCircle.TextColor3 = COLORS.text
    introCircle.TextSize = 34
    introCircle.Font = Enum.Font.GothamBold
    introCircle.TextTransparency = 1
    introCircle.ZIndex = 101
    introCircle.Parent = overlay
    addCorner(introCircle, 60)
    addStroke(introCircle, COLORS.accent, 2)

    task.spawn(function()
        TweenService:Create(credit, TweenInfo.new(0.35, Enum.EasingStyle.Quad,
            Enum.EasingDirection.Out), { TextTransparency = 0 }):Play()
        task.wait(1.05)
        TweenService:Create(credit, TweenInfo.new(0.3, Enum.EasingStyle.Quad,
            Enum.EasingDirection.In), { TextTransparency = 1 }):Play()
        TweenService:Create(introCircle, TweenInfo.new(0.35, Enum.EasingStyle.Back,
            Enum.EasingDirection.Out), { TextTransparency = 0 }):Play()
        task.wait(0.55)
        local targetPosition = dButton.Position
        local targetSize = dButton.Size
        TweenService:Create(introCircle, TweenInfo.new(0.6, Enum.EasingStyle.Quart,
