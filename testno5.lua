...
local function refreshOrbAppearance(data)
    ...
    elseif fraction < 0.35 then
        ...
        -- Saved orb rigs are intentionally invisible.  The HP color is kept
        -- on the part for debugging/inspection without rendering a marker.
        setOrbVisibility(data, false)
    else
...
local function getEntityAimPart(entity)
    ...
    local useSitReference =
        entity:IsA("Player") and S.playerSitRotation
        or (not entity:IsA("Player") and not entity:IsA("BasePart") and S.npcSitRotation)
    if useSitReference then
        local hum = model:FindFirstChildWhichIsA("Humanoid", true)
        local sitRoot = model:FindFirstChild("HumanoidRootPart", true)
        if hum and hum.Sit and sitRoot and sitRoot:IsA("BasePart") then
            return sitRoot
        end
    end
